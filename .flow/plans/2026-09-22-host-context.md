...
## After the merge — not a task, and it takes both machines
...
**`~/dotfiles` is not in the mutagen set** — `~/projects`, `~/work`,
`~/obsidian` and `~/.claude/projects` are, dotfiles is not. So the other machine
needs `git pull && dotter deploy` of its own, run there. Until it has them, its
turns carry no `[CTX]` line while this machine's do, inside transcripts the two
share — and an unmarked turn sitting next to a marked one reads as "same
machine", which is the failure this change exists to prevent. The mark is
trustworthy only once both machines have deployed; on the laptop, verify with
the same command and expect `[CTX] laptop · `.
